...
public class ContentRecommender {
    ...
    public ArrayList<Recommendation> recommend(User user,Feature[] features)
    {
        ...
        if(categories==null||categories.length==0) {
            if (user != null) {
                ...
                cs = user.categories.toArray(new Feature[user.categories.size()]);
            }
        }
        else
        {
            cs=new Feature[categories.length];

            for (int i = 0; i < cs.length; i++) {
                try{
                    cs[i]= Feature.finder.where().eq("name",categories[i]).findList().get(0);
                }
                catch (Exception ex)
                {
        ...
        {
            String[] bids;
            String[] bidsPrev=getAllPosibleSimilarBusiness(user ==null?null: user.user_id,cs);
            ...
            {
                List<Feature> clist = Business.find.byId(bids[i]).categories;
    ...
    public String[] getAllPosibleSimilarBusiness(String uid, Feature[] cs)
    {
        String theQuery="";
        if(cs.length>0)
        {
            ...
            theQuery="select distinct business_business_id from usercategories join businesscategories on businesscategories.category_category_id=usercategories.category_category_id "+
    ...
    private String calculateJoinGeoFiltered() {
        if(filteredBusinessGeo.size()>0)
        {
            ...
            String join=" join (select business_id from movie where "+where+") bfilter on bfilter.business_id=business_business_id";
            return join;
        }
        return "";
    }
    ...
    private String[] getSplitedCategories(User u, double trainingPercentage) {
        ...
        ArrayList<Feature> cats=new ArrayList<Feature>();

        for (int i = 0; i < toplimit; i++) {
            Business b=bs.get(i);
            trainingBusiness.add(b.business_id);
            for (int j = 0; j < b.categories.size(); j++) {
                Feature cate=b.categories.get(j);
